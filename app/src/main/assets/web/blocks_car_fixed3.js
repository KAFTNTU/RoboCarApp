...
window.CAR_CATEGORY = `
<category name="🚗 Машинка" colour="#0062BA">
...
// (removed) 4-motor block deleted (UI cleanup)
...
Blockly.Blocks['loop_forever'] = {
    init: function() {
        ...
        this.setNextStatement(true);
        this.setColour(40);
        this.setTooltip('Виконує вкладені блоки безкінечно (поки не натиснеш Stop).');
    }
};
...
javascript.javascriptGenerator.forBlock['loop_forever'] = function(block) {
    ...
    // Extra stop-check each iteration so infinite loops can be stopped
    return `while (true) {
  if (window._shouldStop) throw "STOPPED";
${branch}}
`;
};
...
Blockly.Blocks['track_action'] = {
  init: function() {
    this.appendDummyInput('HEAD')
      ...
      .appendField(new Blockly.FieldDropdown([
        ["🔴 почати запис", "REC"],
        ["▶️ відтворити 1 раз", "PLAY1"],
        ["🔄 відтворити N разів", "PLAYN"]
      ], (v) => { this.updateShape_(); return v; }), "ACTION");
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setColour(260);
    this.updateShape_();
  ...
  updateShape_: function() {
    ...
    if (act === 'PLAYN') {
      if (!this.getInput('TIMES')) {
        this.appendValueInput("TIMES").setCheck("Number").appendField("разів");
      }
    } else {
      if (this.getInput('TIMES')) this.removeInput('TIMES');
    }
  }
};

javascript.javascriptGenerator.forBlock['track_action'] = function(block) {
...
Blockly.Blocks['start_line_action'] = {
  init: function() {
    this.appendDummyInput()
      .appendField("🏁 Старт-лінія")
      .appendField(new Blockly.FieldDropdown([
        ["чекати", "WAIT"],
        ["зупинитися на лінії", "STOP"]
      ]), "MODE");
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setColour(40);
...
javascript.javascriptGenerator.forBlock['start_line_action'] = function(block) {
...
// === UI POLISH: force block colours to match their category ===
(function(){
  try {
    const pickHue = (key, fallback) => {
      const v = (Blockly && Blockly.Msg && Blockly.Msg[key]) != null ? Blockly.Msg[key] : fallback;
      const n = Number(v);
    ...
    const COLORS = {
      MOVE: '#0062BA',
      CONTROL: '#FFBF00',
      LOGIC: pickHue('LOGIC_HUE', 210),
      LOOPS: pickHue('LOOPS_HUE', 120),
      MATH:  pickHue('MATH_HUE', 230),
    };

    const MAP = [
      { color: COLORS.MOVE, types: [
        'start_hat','robot_stop','robot_move','robot_move_soft','robot_set_speed',
        'robot_turn_timed','motor_single','go_home','track_action','start_line_action'
      ]},
      { color: COLORS.LOGIC, types: [
        'sensor_get','state_set','state_set_reason','state_if','state_prev','state_get',
        'state_time_s','state_enter_count','logic_edge_detect','logic_schmitt'
      ]},
      { color: COLORS.LOOPS, types: [
        'loop_forever','loop_repeat_pause','loop_every_seconds','count_laps'
      ]},
      { color: COLORS.MATH, types: [
        'math_number_limited','math_speed_cms','math_path_vt','math_pid','math_smooth'
      ]},
      { color: COLORS.CONTROL, types: [
    ...
    function forceColor(type, color) {
      const def = Blockly && Blockly.Blocks ? Blockly.Blocks[type] : null;
      if (!def || typeof def.init !== 'function') return;
      if (def.__rcColorForced) return;
      const orig = def.init;
      def.init = function() {
        orig.call(this);
        try { this.setColour(color); } catch (e) {}
      };
      def.__rcColorForced = true;
    }

    for (const g of MAP) {
      for (const t of g.types) forceColor(t, g.color);
    }
  } catch (e) {}
})();
